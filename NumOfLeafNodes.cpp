...
int heightOfATree(TreeNode<int>* root)
{
  ...
  for(int i=0;i<root->children.size();i++)
  {
    ht=max(heightOfATree(root->children[i]),ht);
  }
  return ht+1;
}
void atKthLevelNodes(TreeNode<int>* root,int k)
{
  if(!root) return;
  if(k==0)
  {
  ...
  for(int i=0;i<root->children.size();i++)
  {
    atKthLevelNodes(root->children[i],k-1);
...
int main()
{
  ...
  cout<<heightOfATree(root)<<e;
  cout<<"=======kthlevel====="<<e;
  atKthLevelNodes(root,1);
  cout<<"=========ctLeafNodes============="<<e;
